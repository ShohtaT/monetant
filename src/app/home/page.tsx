...
export default function Page() {
  ...
  const fetchExpandedPayments = async () => {
    ...
    const expandData: ExpandPayment[] = await Promise.all(
  ...
  const completedPayments = expandedPaymentsData.filter(
        ...
        {awaitingPayments.map((payment) => (
          ...
          >
            <div className="flex justify-between items-start">
              <p>{payment.payment_at.slice(0, 10)}</p>
              <div className="p-1 border border-orange-500 text-orange-500 rounded">未完了</div>
            </div>
            <p className="text-xl font-bold">{payment.title}</p>
            <p className="mt-2.5">
      ...
      <ul className="mt-4">
        {completedPayments.map((payment) => (
          ...
          >
            <div className="flex justify-between items-start">
              <p>{payment.payment_at.slice(0, 10)}</p>
              <div className="p-1 border border-green-500 text-green-500 rounded">完了</div>
            </div>
            <p className="text-xl font-bold">{payment.title}</p>
            <p className="mt-2.5">
